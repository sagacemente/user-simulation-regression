...
def fit_model(xtrain, ytrain, xtest, ytest,
              FILTER_SIZE=32 , KERNEL_SIZE=8, STRIDE=8,POOL_SIZE=2,
    ...
    #print('input_user.shape',input_user.shape)
    cnnout = tf.keras.layers.Conv1D(filters=FILTER_SIZE,
                                    kernel_size=KERNEL_SIZE,
                                    strides=STRIDE,
                                    padding="valid")(input_user)
    max_pool_1d = tf.keras.layers.MaxPooling1D(pool_size=POOL_SIZE,
                                               strides=STRIDE,
                                               padding='valid')(cnnout)
    #whole_seq_output, final_memory_state, final_carry_state = layers.LSTM(30, return_sequences=True, return_state=True)(input_user)
    flattened = tf.keras.layers.Flatten()(max_pool_1d)
    dense1 = layers.Dense(hidden_dim1, activation=activation, name='dense1')(flattened)  #(dense1)
...
model, hist = fit_model(xtrain, ytrain, xtest, ytest,
              hidden_dim1=32,hidden_dim2=16,drop_out=0.005,optimizer='adam',loss=LOSS,activation ='swish',
              batch_size=160,epochs=1000,early_stop_patience = 800,
              PLOT=True, name = 'model', save_model=False)
